...
function subdomainVisits(cpdomains: string[]): string[] {
	...
	for (let i = 0; i < cpdomains.length; i++) {
		findAllSubdomains(cpdomains[i], map);
	}
	const a = [...map.entries()].map(([key, value]) => `${value} ${key}`);
	return a;
}
function findAllSubdomains(str: string, map: Map<string, number>) {
	const [count, domain] = str.split(" ");
	const domains = domain.split(".");
	for (let i = 0; i < domains.length; i++) {
		const tempArr: string[] = [];
		for (let j = i; j < domains.length; j++) {
			tempArr.push(domains[j]);
		}
		map.set(tempArr.join("."), (map.get(tempArr.join(".")) || 0) + Number(count));
	}
}
// example 1
